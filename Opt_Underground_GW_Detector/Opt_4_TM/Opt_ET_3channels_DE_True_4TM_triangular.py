...
workers = workers parameter of the Differential Evolution function (to parallelize the optimization)
...
def foo(N=10, hh=0, worker=1):
        ...
        # bound = [(-500., 500.)]
        # x_bound = bound*N*D
        bound = [(-100., 700.), (-300., 700.), (-300., 300.)]
        x_bound = bound*N
        
        
        #Parameters to be passed to the Residual function
        Res_Par = (N, freq, SNR, p)
        


        # **************************** DIFFERENTIAL EVOLUTION ALGORITHM *************************************
        print('starting proc ...')
        Final_State = differential_evolution(Residual, x_bound, Res_Par, disp=True, maxiter=4500, popsize=65, init='random', workers=worker, recombination=0.75, mutation=(0., 1.5), strategy='best1bin', tol=0.001, updating='deferred')
        # print (Final_State.x)
        ...
        json.dump(np.sqrt(Final_State.fun),f)
        f.write('\n')
