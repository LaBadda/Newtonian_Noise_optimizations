...
def Residual (state, N, freq, SNR, p):
        ...
        #resid = 1-np.dot(Csn,linalg.inv(Css)).dot(Csn)/Cnn
...
def foo(N,hh):
        ...
        t0 = clock()
        print('starting proc ...')
        Final_State = differential_evolution(Residual, x_bound, Res_Par, disp=True, maxiter=4500, popsize=40)#,  recombination=0.5, mutation=(0.5, 1), strategy='best2bin')
        t1 = clock()
